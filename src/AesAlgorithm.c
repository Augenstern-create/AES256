...
int HWEIOS_RIJKEXH262F(unsigned long *rk, const unsigned char *key,
                         int keybits);

int HWEIOS_RIJKEXH263F(unsigned long *rk, const unsigned char *key,
                         int keybits);
...
#define Te0 GlsgleJsona
#define Te1 GlsgleJsonb
#define Te2 GlsgleJsonc
#define Te3 GlsgleJsond
#define Te4 GlsgleJsone

#define Td0 GlsgleJsonf
#define Td1 GlsgleJsong
#define Td2 GlsgleJsonh
#define Td3 GlsgleJsonk
#define Td4 GlsgleJsons
...
#define GETU32(plaintext)                                                                                              \
    (((u32)(plaintext)[0] << 24) ^ ((u32)(plaintext)[1] << 16) ^ ((u32)(plaintext)[2] << 8) ^ ((u32)(plaintext)[3]))
...
/// @param plaintext
/// @return
static u32 U8ToU32(u8 *plaintext) {
    return ((u32)plaintext[0] << 24) | ((u32)plaintext[1] << 16) |
           ((u32)plaintext[2] << 8) | ((u32)plaintext[3]);
}

/// @brief 32位无符号整数分解为4字节
/// @param ciphertext
/// @param st
static void U32ToU8(unsigned char *ciphertext, unsigned int st) {
    ciphertext[0] = (unsigned char)(st >> 24);
 ...
 */
int HWEIOS_RIJKEXH262F(u32 *rk, const u8 *key, int keybits) {
    int i = 0;
    u32 temp;
 ...
 * @return the number of rounds for the given cipher key size.
 */
int HWEIOS_RIJKEXH263F(u32 *rk, const u8 *key, int keybits) {
    int nrounds, i, j;
    u32 temp;

    /* expand the cipher key: */
    nrounds = HWEIOS_RIJKEXH262F(rk, key, keybits);
...
void HWEIOS_RIJEAEXH265F(const u32 *rk, int nrounds, const u8 plaintext[16],
                     u8 ciphertext[16]) {
...
void HWEIOS_RI64F(const u32 *rk, int nrounds, const u8 ciphertext[16],
                     u8 plaintext[16]) {
    u32 s0, s1, s2, s3, t0, t1, t2, t3;
#ifndef FULL_UNROLL
    int r;
...
#define CODE_RESULT_BUFFER 1024 * 2024 * 10

// static unsigned char net_work_json_key_word[] = {0x1a, 0xf6, 0xa5, 0xa3,
// 0xbe, 0x15, 0x38, 0x46, 0xf5,0xfe,0x0e,0x1e,0xe5, 0x63}; void
// write_json_key_word(const char *ptrPath, void *ptrData, int len) {
//
//     int file_id = open(ptrPath, O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR);
//     int ret=write(file_id, ptrData, len);
//     LOGE("test save data: %s",ptrPath);
//     close(file_id);
// }

static char *gJsonKeyWordPtr = NULL;
static int gJsonKeyWordLen = 0;

/**
 * 读取加密秘钥; 这里用不到;
 * @param ptrPath
 */
// static void read_json_key_word(const char *ptrPath) {
//     if (gJsonKeyWordPtr == NULL) {
//         gJsonKeyWordLen = get_file_size(ptrPath);
//         FILE *read      = fopen(ptrPath, "r+");
//         gJsonKeyWordPtr = (char *)malloc(gJsonKeyWordLen);
//         memset(gJsonKeyWordPtr, 0, gJsonKeyWordLen);
//         fread(gJsonKeyWordPtr, gJsonKeyWordLen, 1, read);
//         fclose(read);
//     }
// }

// void set_json_key_work_data(char *keyPtr, int keyLen)
//{
//     gJsonKeyWordPtr = keyPtr;
//     gJsonKeyWordLen = keyLen;
// }
 ...
 * @return 返回的指针长度;
 */
int aes_encode_data2(char *ptrData, int dataLen, unsigned char *resultBuffer) {
    ...
    if (gJsonKeyWordPtr == NULL || gJsonKeyWordLen == 0) {
        // LOGE("read json key word error. maybe key path error.");
        return 0;
    }

    for (i = 0; i < gJsonKeyWordLen; i++) {
        key[i] = gJsonKeyWordPtr[i];
    }

    for (i = gJsonKeyWordLen; i < (int)sizeof(key); i++) {
        key[i] = 0;
    }

    if (dataLen > CODE_RESULT_BUFFER) {
        // LOGE("license data is bigger then max size.error!");
        return -1;
    }
    ...
    // 第三步:开始解码;
    copyCnt = 0;
    nrounds = HWEIOS_RIJKEXH262F(rk, key, 256);

    while (copyCnt < dataLen) {
        unsigned char plaintext[16];
        unsigned char ciphertext[16];

        memcpy(ciphertext, &ptrData[copyCnt], 16);  // 每次拷贝16个字节.
        copyCnt += 16;

        HWEIOS_RIJEAEXH265F(rk, nrounds, ciphertext, plaintext);

        memcpy(&resultBuffer[writeIndex], plaintext, sizeof(plaintext));
        writeIndex += sizeof(plaintext);
    }
...
/**
 * aes解码数据;
 * @param data data一定是16的倍数,解密.
 * @param dataLen
 * @param resultBuffer
 * @return
 */
static int aes_decode_data(char *data, int dataLen,
    ...
    // 开始解码;
    copyCnt = 0;
    nrounds = HWEIOS_RIJKEXH263F(rk, key, 256);

    while (copyCnt < dataLen) {
        unsigned char plaintext[16];
        unsigned char ciphertext[16];

        memcpy(ciphertext, &data[copyCnt], 16);  // 每次拷贝16个字节.
        copyCnt += 16;

        HWEIOS_RI64F(rk, nrounds, ciphertext, plaintext);
...
static int encode_json_string2(char *jsonString, int dataLen,
                               unsigned char *resultBuffer) {
    unsigned long rk[RKLENGTH(256)];
    unsigned char key[KEYLENGTH(256)];
    int i;
    ...
    while (copyCnt < dataLen) {
        ...
        memcpy(ciphertext, &jsonString[copyCnt], 16);  // 每次拷贝16个字节.
        copyCnt += 16;
...
/**
 * hexString 转值数组; 每两个英文字母组合成一个char 8位字节;
 * @param pbDest
 * @param pbSrc
 * @param nLen
 */
static void StrToHex2(char *pbDest, char *pbSrc, int nLen) {
    char h1, h2;
    char s1, s2;
    int i;
 ...
 * @param nLen
 */
static void HexToStr(char *pbDest, unsigned char *pbSrc, int nLen) {
    char ddl, ddh;
    int i;
    ...
    for (i = 0; i < nLen; i++) {
        ...
        if (ddh > 57 && ddh < 96) ddh = ddh + 7;
        if (ddl > 57 && ddl < 96) ddl = ddl + 7;
...
/**
 *
 * @param ptrHexString  要解码的加密字符串字节;
 * @param keyPtr 秘钥指针
 * @param keyLen 秘钥指针长度
 * @return
 */
char *DecodeHexJsonWithKeyPtr(char *ptrHexString, char *keyPtr, int keyLen) {
    unsigned char *ptrDecodeBuffer = (unsigned char *)malloc(1024 * 10 * 10);
    memset(ptrDecodeBuffer, 0, 1024 * 10 * 10);

    gJsonKeyWordPtr = keyPtr;
    gJsonKeyWordLen = keyLen;

    int hexLen = strlen(ptrHexString) + 1;  // 这里不再除以2;

    char *ptrHex = (char *)malloc(hexLen);
    memset(ptrHex, 0, hexLen);

    StrToHex2(ptrHex, ptrHexString, strlen(ptrHexString));

    int ret = aes_decode_data(ptrHex, hexLen, ptrDecodeBuffer);
    //        if(ret!=hexLen-1){
    //                LOGE("decode_hexString_to_jsonString ERROR, input hex len
    //                != decode len...");
    //        }
    free(ptrHex);

    // 把解码的数据拷贝出去;
    char *retHex = (char *)malloc(ret + 1);
    memset(retHex, 0, ret + 1);
    memcpy(retHex, ptrDecodeBuffer, ret);
    free(ptrDecodeBuffer);
    return retHex;
}
 ...
 * @return
 */
char *LSOEncodeDataWithKey(char *dataPtr, int dataLen, char *keyPtr, int keyLen,
                           int *retLen) {
    if (dataPtr == NULL || dataLen <= 0 || keyPtr == NULL || keyLen <= 0) {
        return NULL;
    }
    ...
    unsigned char *ptrEncodeBuffer =
        (unsigned char *)malloc(sizeof(char) * (dataLen + 16 + 1));
    if (ptrEncodeBuffer)
        memset(ptrEncodeBuffer, 0, sizeof(char) * (dataLen + 16 + 1));
    int ret = aes_encode_data2(dataPtr, dataLen, ptrEncodeBuffer);
    char *ptrHex = (char *)malloc(sizeof(char) * (ret * 2 + 1));
    if (ptrHex) memset(ptrHex, 0, sizeof(char) * (ret * 2 + 1));
    if (ret > 0) {
        HexToStr(ptrHex, ptrEncodeBuffer, ret);
    }
    free(ptrEncodeBuffer);
    return ptrHex;
}
...
/// @param retLen
/// @return
char *LSODecodeDataWithKey(char *dataPtr, int dataLen, char *keyPtr, int keyLen,
                           int *retLen) {
    unsigned char *ptrDecodeBuffer = (unsigned char *)malloc(1024 * 10 * 10);
    memset(ptrDecodeBuffer, 0, 1024 * 10 * 10);
    ...
    int hexLen = dataLen + 1;

    char *ptrHex = (char *)malloc(hexLen);
    memset(ptrHex, 0, hexLen);

    StrToHex2(ptrHex, dataPtr, dataLen);
...
char *decode_data_with_key(char *dataPtr, int dataLen, char *keyPtr, int keyLen,
                           int *retLen) {
    char *ptrDecodeBuffer = malloc(1024 * 10 * 10);
    memset(ptrDecodeBuffer, 0, 1024 * 10 * 10);

    gJsonKeyWordPtr = keyPtr;
    gJsonKeyWordLen = keyLen;

    int ret = aes_decode_data(dataPtr, dataLen, ptrDecodeBuffer);

    // 把解码的数据拷贝出去;
    char *retHex = malloc(ret + 1);
    memset(retHex, 0, ret + 1);
    memcpy(retHex, ptrDecodeBuffer, ret);
...
#define CODE_RESULT_BUFFER 1024 * 2024 * 3
//"L"的
#define XOR_HEX 108

static unsigned char shaderPassword[] = {
    0x26, 0xa6, 0xf5, 0xae, 0xb6, 0x85, 0xc3, 0xec, 0xa5, 0x63,
    0xa5, 0xb3, 0x7c, 0x65, 0x03, 0xaf, 0x73, 0x70, 0x88, 0x58};

static int resultBufferCnt = 0;

//"L"的
static void copyToResultBuffer(unsigned char *pSrc, int len,
                               unsigned char *resultBuffer) {
    int i = 0;
    if ((resultBufferCnt + len) > CODE_RESULT_BUFFER) {
        // LOGE("copy to result buffer error, lenght is bgger than len");
        return;
    }
    memcpy(&resultBuffer[resultBufferCnt], pSrc, len);
    resultBufferCnt += len;
}

/*
 data一定是16的倍数,解密.
 data: 输入的是:一个文件中的全部数据;
*/
int decoderShader(char *data, int dataLen, unsigned char *resultBuffer) {
    ...
    int nrounds;
    int copyCnt = 0;
    unsigned char ADV_XOR = 0;  // 高级版本校验值.

    // passwd的大小一定小于sizeof(key)
    for (i = 0; i < (int)sizeof(shaderPassword); i++) {
        key[i] = shaderPassword[i];
    }

    for (i = sizeof(shaderPassword); i < (int)sizeof(key); i++) {
        key[i] = 0;
    }
    ...
    while (copyCnt < dataLen) {
        ...
        HWEIOS_RI64F(rk, nrounds, ciphertext, plaintext);
        copyToResultBuffer(plaintext, sizeof(plaintext),
                           resultBuffer);  // AES后再放到buffer中.
    }
    return 0;
}